...
public class Baeskinrabins31string {
	public static void main(String[] args) {
		...
		while(st) {
			...
			if(count < 32) {
				...
				if(count > 10 ) {
					str = String.valueOf(input.charAt(input.length() - 2));
					str += String.valueOf(input.charAt(input.length() - 1));
					count = Integer.valueOf(str);
					count++;
				} else {
					 
					 count = Integer.valueOf(String.valueOf(input.charAt(input.length() - 1)));
					 count++;
				}
